...
2. Do not add overlapping prose twins. `manuscript-writing` is the manuscript prose and audit source of truth; `scientific-writing` is a deprecated alias only.
3. `discover-papers` must support quick mode without PROTOCOL.md.
4. `protocol` must remain a real, independently invocable skill.
...
```bash
./validate-skills.sh
python -m unittest tests/test_validator_fixture.py
```
